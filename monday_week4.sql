...
-- Let's got back to the example where we used a HAVING clause

SELECT Location, MAX(Area) AS MaxArea
    FROM Site
...
-- What is a join? Conceptually, the database performs a "Cartesian product" of the tables, then matches up the rows based on some kind of join condition,
...
-- Here's where the Cartesian product looks like:
SELECT * FROM A CROSS JOIN B;

-- Let's add a join condition, which can be *any* expression!
-- This is what's returned to as an INNER JOIN
...
-- Left join: we're adding rows from the other table that never got matched.
...
-- One benefit of joining on a column that has the same name (i.e., House_ID here) is that you can use USING clause.
SELECT * FROM Student JOIN House USING (House_ID);

-- Now back in the bird database.
...
SELECT * FROM (SELECT * FROM Bird_eggs ORDER BY Width) JOIN Bird_nests USING (Nest_ID);

-- Gotcha with DuckDB.. It's not as smart as some other databases.
SELECT Nest_ID, COUNT(*) FROM Bird_nests JOIN Bird_eggs USING (Nest_ID) GROUP BY Nest_ID;

-- Some databases allow you to say:
SELECT Nest_ID, COUNT(*) FROM Bird_nests JOIN Bird_eggs USING (Nest_ID) GROUP BY Nest_ID; -- Should give an error.

-- Workaround:
SELECT Nest_ID, ANY_VALUE(Species), COUNT(*) FROM Bird_Eggs JOIN Bird_nests USING (Nest_ID) GROUP BY Nest_ID;

SELECT Nest_ID, Species, COUNT(*) FROM Bird_Eggs JOIN Bird_nests USING (Nest_ID) GROUP BY Nest_ID, Species;

SELECT Nest_ID, Species, Width, Length FROM Bird_eggs JOIN Bird_nests USING (Nest_ID) LIMIT 10;

SELECT Nest_ID, Species, Egg_num, Width, Length FROM ORDER BY Nest_ID, Egg_num LIMIT 10;

-- Can two species inhabit the same nest?
.table Bird_nests

-- ANY_VALUE literally returns any value
SELECT Nest_ID, ANY_VALUE(Width) FROM Bird_eggs GROUP BY Nest_ID;
