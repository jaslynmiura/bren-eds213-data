...
-- but just more compacts to say USING (Nest_ID)
-- you can get away with *not* prefixing columns if they're unambiguous
-- but if the names are ambiguous, you need the prefix them

-- Note on UNION: SQL will UNION any two tables that have the same number of columns and compatible data types
...
INSERT INTO Personnel VALUES ('gimiu', 'Gigi Miura');
SELECT * FROM Personnel;

-- Good practice for safer code: name the columns
INSERT INTO Personnel (Abbreviation, Name) VALUES ('avarob', 'Ava Robillard');
-- Also, when you insert a row in a table, you don't necessarily have to specify all the values; anything not specified will either be filled with NULL or a default value
...
-- Another idea:
-- Use a fake table name, then cahnge to the real name
